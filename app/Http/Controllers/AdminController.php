...
class AdminController extends AdminBaseController
{
    ...
    public function configurePaymentGatewayPost(Request $request)
    {
        ...
        } elseif ($api_name === "toyyibpay") {
            $payment_gateway->name = "toyyibpay";
            $payment_gateway->api_name = $api_name;
            $payment_gateway->private_key = $request->private_key;
            $payment_gateway->public_key = $request->public_key;
    ...
    public function studentprenuer()
    {
        $data = studentprenuer::all();
        return view("admin.studentprenuer")->with(array('data' =>$data));
    }
    public function addstudentprenuer(Request $request)
    {
        $file = $request->image;
        $filename = rand() . '.' . $file->getClientOriginalExtension();
        $file->move(public_path('images'), $filename);
        
        $newimage = new studentprenuer();
        $newimage->image = $filename;
        $newimage->name = $request->name;
        $newimage->bakery_name = $request->bakery_name;
        $newimage->save();

        return back()->with('success','Studentprenuer Added succesfully');
    }
    public function editstudentprenuer(Request $request)
    {
        $newimage = studentprenuer::find($request->id);
        if ($request->image) {
            $file = $request->image;
            $filename = rand() . '.' . $file->getClientOriginalExtension();
            $file->move(public_path('images'), $filename);
        ...
        return back()->with('success','Studentprenuer Update succesfully');
    }
    
    public function deletestudentprenuer(Request $request)
    {
        $studentprenuer_delete = studentprenuer::where('id','=',$request->id)->delete();
        if($studentprenuer_delete==true){
            return back()->with('success','Studentprenuer Delete successfull');
        }else{
    ...
    public function cookiePage()
    {
        ...
        return \view("admin.cookie-page-editor", [
            "selected_navigation" => "landing-page",
